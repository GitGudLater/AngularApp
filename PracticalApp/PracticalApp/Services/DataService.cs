...
namespace PracticalApp.Services
{
    public class DataService:IDataService
    {
        ...
        public IEnumerable<FullList> GetPhones(string username, bool personal)
        {
            switch (username)
            {
                case null:
                ...
                default:
                    ...
                    {
                        case true:
                            return GetList(username, personal);
                        default:
        ...
        private IEnumerable<FullList> GetList(string username, bool personal)
        {
            ...
            {
                User user = products[i].Users.First(x => x.Email == username);
                if (user != null)
                    fullList.Add(new FullList() { Id = products[i].Id, Name = products[i].Name, Designer = products[i].Designer, About = products[i].About, Cost = products[i].Cost });
            }
            return fullList;
        }

        private IEnumerable<FullList> GetList(string username)
        {
            List<Product> products = db.Products.ToList();
            List<FullList> fullList = new List<FullList>();
            int count = products.Count;
        ...
        private IEnumerable<FullList> GetList()
        {
            List<Product> products = db.Products.ToList();
            List<FullList> fullList = new List<FullList>();
            int count = products.Count;
