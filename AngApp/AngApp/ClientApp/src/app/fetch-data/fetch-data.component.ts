...
@Component({
  ...
  providers: [DataService, RelationService],
})
export class FetchDataComponent implements OnInit {
  //!!!!!Реализовать значение чекбокса при загрузке компонента
  ...
  constructor(private dataService: DataService, private relationService: RelationService, http: HttpClient, @Inject('BASE_URL') baseUrl: string) {
  ...
  check(id: number,useles:string) {
    ...
    this.relationService.checkRelation(this.relation);
  }
  ...
  add(p: Product) {
    this.dataService.createProduct(p);
    this.cancel();
